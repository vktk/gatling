...
class HttpCompileTest extends Simulation {
  ...
  val lambdaUser = scenario("Standard User")
    ...
    .pause(pause1)
    .exec(polling("poll").every(10.seconds).exec(http("poll").get("/foo")))
    .exec(polling.pollingName("poll").every(10.seconds).exec(http("poll").get("/foo")))
    .exec(polling("poll").stop)
    .exec(polling.pollingName("poll").stop)
    .rendezVous(100)
