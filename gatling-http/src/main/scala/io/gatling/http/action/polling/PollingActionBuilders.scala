...
class PollingStartBuilder(
  ...
  requestBuilder: HttpRequestBuilder)(implicit defaultHttpProtocol: DefaultHttpProtocol, httpEngine: HttpEngine)
  ...
  override def build(system: ActorSystem, next: ActorRef, ctx: ScenarioContext) = {
    ...
    system.actorOf(PollingStartAction.props(pollingName, duration, requestDef, ctx.dataWriters, next), actorName("pollingStart"))
  }
}
